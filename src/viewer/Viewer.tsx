import { BufferGeometryUtils, LineMaterial, LineSegments2, LineSegmentsGeometry } from 'three/examples/jsm/Addons.js'
...
function Viewer({ style, store, activeView }: ViewerProps) {
  ...
  const mergedGeometry = useMemo(() => {
    ...
    const merged = BufferGeometryUtils.mergeGeometries(dtoGeometries) ?? new THREE.BufferGeometry()

    let offset = 0
    const faceMap: Record<number, string> = {}
    for (const dto of dtos) {
      const dtoGeometry = dto.geometry()
      const faceCount = dtoGeometry.index ? dtoGeometry.index.count / 3 : dtoGeometry.attributes.position.count / 3
  ...
  const saveCameraState = useCallback(() => {
    ...
    saveStateTimer.current = setTimeout(() => {
      if (!activeViewId || !cameraControlRef.current || isApplyingViewChange.current) {
        return
      }
      ...
      updateViewPosition(activeViewId, [position.x, position.y, position.z], [target.x, target.y, target.z], cameraUp)
    }, 100)
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Tab') {
        ...
        if (ind != undefined && ind != null) setHoveredGUID(mergedGeometry?.userData?.faceMap?.[ind])
  ...
  useEffect(() => {
    if (intersectionSphereRef.current) intersectionSphereRef.current.layers.set(1)
  }, [intersectionPoint])
  //#endregion
  ...
  const handleMouseMove = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      const intersects = raycaster.intersectObjects(scene.children)
      if (intersects.length > 0) {
        setHoveredObjects(intersects.map((i) => i.faceIndex).filter((i) => i != undefined && i != null))
        let guid = undefined
        const ind = intersects[0].faceIndex
        if (ind != undefined && ind != null) {
          guid = (intersects[0].object as THREE.Mesh).geometry.userData.faceMap?.[ind]
        }
    ...
    [getMouse, Hover.Enabled, setHoveredGUID, setHoverIndex, setHoveredObjects, setIntersectionPoint],
  )
  //#endregion
    ...
    if (viewData && cameraControlRef.current) {
      ...
      if (settings.constraints) {
        if (settings.constraints.azimuthRotateSpeed !== undefined)
          cameraControlRef.current.azimuthRotateSpeed = settings.constraints.azimuthRotateSpeed
        if (settings.constraints.polarRotateSpeed !== undefined)
          cameraControlRef.current.polarRotateSpeed = settings.constraints.polarRotateSpeed
        ...
        if (settings.constraints.draggingSmoothTime !== undefined)
          cameraControlRef.current.draggingSmoothTime = settings.constraints.draggingSmoothTime
  ...
  return (
    <div style={containerStyles} ref={containerRef}>
      ...
      >
        ...
        <directionalLight position={[100 * Math.cos(angle), 100 * Math.sin(angle), 0]} intensity={Math.PI} />
        <mesh geometry={mergedGeometry} material={mergedGeometryMaterial}></mesh>

        {selectedOutlineGeometry && (
          <primitive
            object={new LineSegments2(selectedOutlineGeometry, selectedOutlineMaterial)}
            layers={1}
            renderOrder={2}
          />
        )}
        {hoveredOutlineGeometry && (
          <primitive
            object={new LineSegments2(hoveredOutlineGeometry, hoveredOutlineMaterial)}
            layers={1}
            renderOrder={3}
          />
        )}
        {intersectionPoint && (
          <mesh ref={intersectionSphereRef} position={[intersectionPoint.x, intersectionPoint.y, intersectionPoint.z]}>
        ...
        <CameraControls
          ...
          azimuthRotateSpeed={
            cameraConstraints.azimuthRotateSpeed !== undefined ? cameraConstraints.azimuthRotateSpeed : 1.0
          }
          ...
          draggingSmoothTime={
            cameraConstraints.draggingSmoothTime !== undefined ? cameraConstraints.draggingSmoothTime : 0
          }
          smoothTime={cameraConstraints.smoothTime !== undefined ? cameraConstraints.smoothTime : 0}
          onUpdate={handleCameraUpdate}
        />
        {Attributes.Viewer.GridHelper && <gridHelper args={[20, 20, '#888888', '#444444']} raycast={() => {}} />}
      </Canvas>
    </div>
  )
}
