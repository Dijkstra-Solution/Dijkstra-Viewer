...
export function useEventEmitter() {
  ...
  const on = useCallback(<T extends EventType>(event: T, callback: EventCallback<EventPayloads[T]>) => {
    if (!listeners.current.has(event)) listeners.current.set(event, new Set())
    listeners.current.get(event)!.add(callback)
  }, [])
  const off = useCallback(<T extends EventType>(event: T, callback: EventCallback<EventPayloads[T]>) => {
    listeners.current.get(event)?.delete(callback)
  }, [])
  const fire = useCallback(<T extends EventType>(event: T, payload: EventPayloads[T]) => {
    listeners.current
      .get(event)
